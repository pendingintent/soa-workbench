...
from ..utils import get_protocol_ct_rows

router = APIRouter()
logger = logging.getLogger("soa_builder.web.routers.protocol_controlled_terminology")
...
@router.get("/protocol-ct/terminology", response_class=JSONResponse)
def get_protocol_ct_terminology(
    ...
    """Query the cached Protocol Controlled Terminology package."""
    limit = max(1, min(limit, 200))
    offset = max(0, offset)

    payload = get_protocol_ct_rows(force=refresh)
...
@router.get(
    "/ui/protocol-ct/terminology",
    response_class=HTMLResponse,
    name="ui_protocol_controlled_terminology",
)
def ui_protocol_controlled_terminology(
    request: Request,
    search: Optional[str] = None,
    code: Optional[str] = None,
    codelist_name: Optional[str] = None,
    ...
    """Detail page for Protocol Controlled Terminology sourced from CDISC Library."""
    try:
        data = get_protocol_ct_terminology(
            search=search,
            code=code,
            codelist_name=codelist_name,
            codelist_code=codelist_code,
    ...
    return templates.TemplateResponse(
        request,
        "protocol_controlled_terminology.html",
        {
            **data,
            "search": search or "",
            "code": code or "",
