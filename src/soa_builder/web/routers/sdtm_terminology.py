...
from ..utils import get_sdtm_ct_rows

router = APIRouter()
logger = logging.getLogger("soa_builder.web.routers.sdtm_terminology")
...
def get_sdtm_terminology(
    ...
    """Query the cached SDTM Controlled Terminology package.
    ...
    payload = get_sdtm_ct_rows(force=refresh)
...
@router.get(
    "/ui/sdtm/terminology", response_class=HTMLResponse, name="ui_sdtm_terminology"
)
def ui_sdtm_terminology(
    request: Request,
    search: Optional[str] = None,
    code: Optional[str] = None,
    codelist_name: Optional[str] = None,
    ...
    """Detail page for SDTM Controlled Terminology sourced from CDISC Library."""
    try:
        data = get_sdtm_terminology(
            search=search,
            code=code,
            codelist_name=codelist_name,
            codelist_code=codelist_code,
